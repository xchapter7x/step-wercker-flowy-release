...
def system_call_mock_next_tag(cmdString):
  ...
  if cmdString == core.version_increment_string(current_tag):
...
def system_call_mock_next_tag_default(cmdString):
  ...
  if cmdString == core.version_increment_string(default_current_tag):
    return core.system_call(cmdString)
...
class CoreTestCase(unittest.TestCase):
  ...
  def test_system_call_success(self):
    ...
    """
    control = ["hi there"]
    o, e = core.system_call("echo "+control[0])
    self.assertFalse(e)
  ...
  def test_make_array_from_stdout(self):
    ...
    del controlResult[-1]
    out = core.make_array_from_stdout(controlString)
  ...
  def test_get_next_tag_default(self):
    ...
    newVersion = core.get_next_tag(system_call_mock_next_tag_default)
    self.assertEqual(newVersion, control)
